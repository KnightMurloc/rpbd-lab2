    ...
    public class EmployeesTab : Tab
    {
        ...
        class Row : ListBoxRow, IRow
        {
            ...
            public Employees Employee
            {
                get
                {
                    Repository.Instance.Refresh(_employees);
                    return _employees;
                }
...
#pragma warning restore 649
        ...
        private EmployeerMenu(Builder builder) : base(builder.GetRawOwnedObject("employees_info"))
        {
            ...
            var posts = Repository.Instance.FindByCondition<Post>("from Post").List<Post>();
        ...
        public bool SettingData(ref Employees employees)
        {
            ...
            if (LastNameEntry.Text.IsEmpty())
            {
                ShowError(_("no last name specified"));
                return false;
            }
            
            if (AddressEntry.Text.IsEmpty())
            {
                ShowError(_("no address specified"));
                return false;
            }
            
            if (DayEntry.Text.IsEmpty())
            {
                ShowError(_("no day specified"));
                return false;
            }
            
            if (MonthEntry.Text.IsEmpty())
            {
                ShowError(_("not month specified"));
                return false;
            }
            
            if (YearEntry.Text.IsEmpty())
            {
            ...
            Post post = postQuery.UniqueResult<Post>();
            ...
            Address address = Repository.Instance.FindByCondition<Address>($"from Address a where a.Name = '{AddressEntry.Text}'").UniqueResult<Address>();
            if (address == null)
            {
                address = new Address();
